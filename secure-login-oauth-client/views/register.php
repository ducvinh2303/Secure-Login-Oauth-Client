...
// set state valid when non-user-loged
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($userController->validateRequestCreateUser($_POST)) {

        $userController->createUser($_POST); // handle create user
    }
}

// variable error and success
$error = "";
$success = "";
...
<body>
    <div class="flex flex-col items-center bg-gray-200 h-screen py-10">
        <div class="max-w-md bg-gray-100 w-full px-8 py-8 rounded-2xl">
            ...
            <div class="font-semibold text-gray-400">Create your account to access my website</div>
            ...
            <form action="" method="post">
                ...
                <div class="mt-3 flex flex-col gap-1">
                    ...
                    <div class="">
                        <input type="password" class="bg-white w-full h-10 border border-solid border-gray-200 rounded outline-none focus:ring-2 ring-sky-400 px-3" required name="re-password" value="<?= htmlspecialchars($_POST['re-password'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                    </div>
                </div>

                <div class="mt-5">
                    <button class="bg-sky-500 hover:bg-sky-600 active:bg-sky-700 text-white w-full h-10 rounded-md font-bold">Register!</button>
                </div>
            </form>

            <div class="mt-3 text-center font-semibold text-gray-600">Already have account? <a href="./login" class="text-sky-600 hover:text-sky-700 active:text-sky-800">Login Now.</a></div>

            <div class="mt-20 text-center font-semibold text-gray-600">Or Sign Up with</div>
